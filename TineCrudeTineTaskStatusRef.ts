...
// Generated Date: 9/9/2018 4:04:33 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineTaskStatusRef {
    'use strict';

    type TineTaskStatusRefType = {
        tineTaskStatusRcd: string,
        tineTaskStatusName: string,
        dateTime: string,
        tineTaskStatusRefPartitionKey: string,
        tineTaskStatusRefRowKey: string
    };

    // declaration
    declare var gettineTaskStatusRefList: TineTaskStatusRefType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineTaskStatusRefTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineTaskStatusRefTiles {
        ...
        constructor(
            ...
            // init tiles
            TineTaskStatusRefTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineTaskStatusRefs
            this.resizeCanvas(14 + 30);
            TineTaskStatusRefTiles.clearCanvas('white');
            ...
            let header: Tile =
                new Tile(
                    ...
                    line,
                    '',
                    gettineTaskStatusRefList.length,
                    14 + 30,
                    15,
                    2,
                    5,
            ...
            // header
            TineTaskStatusRefTiles.tiles.push(header);

            // remaining tineTaskStatusRefs
            var lastTile: Tile;

            for (let i: number = 0; i < gettineTaskStatusRefList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineTaskStatusRefList[i].tineTaskStatusRcd,
                        40,
                        line,
                        gettineTaskStatusRefList[i].userId,
                        gettineTaskStatusRefList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineTaskStatusRefTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineTaskStatusRefTiles.tiles) {
                    for (let tile of TineTaskStatusRefTiles.tiles) {
