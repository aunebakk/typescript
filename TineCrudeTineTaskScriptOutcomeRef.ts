...
// Generated Date: 9/9/2018 4:04:27 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineTaskScriptOutcomeRef {
    'use strict';

    type TineTaskScriptOutcomeRefType = {
        tineTaskScriptOutcomeRcd: string,
        tineTaskScriptOutcomeName: string,
        dateTime: string,
        tineTaskScriptOutcomePartitionKey: string,
        tineTaskScriptOutcomeRowKey: string
    };

    // declaration
    declare var gettineTaskScriptOutcomeRefList: TineTaskScriptOutcomeRefType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineTaskScriptOutcomeRefTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineTaskScriptOutcomeRefTiles {
        ...
        constructor(
            ...
            // init tiles
            TineTaskScriptOutcomeRefTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineTaskScriptOutcomeRefs
            this.resizeCanvas(14 + 30);
            TineTaskScriptOutcomeRefTiles.clearCanvas('white');
            ...
            let header: Tile =
                new Tile(
                    ...
                    line,
                    '',
                    gettineTaskScriptOutcomeRefList.length,
                    14 + 30,
                    15,
                    2,
                    5,
            ...
            // header
            TineTaskScriptOutcomeRefTiles.tiles.push(header);

            // remaining tineTaskScriptOutcomeRefs
            var lastTile: Tile;

            for (let i: number = 0; i < gettineTaskScriptOutcomeRefList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineTaskScriptOutcomeRefList[i].tineTaskScriptOutcomeRcd,
                        40,
                        line,
                        gettineTaskScriptOutcomeRefList[i].userId,
                        gettineTaskScriptOutcomeRefList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineTaskScriptOutcomeRefTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineTaskScriptOutcomeRefTiles.tiles) {
                    for (let tile of TineTaskScriptOutcomeRefTiles.tiles) {
