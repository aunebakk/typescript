...
// Generated Date: 9/9/2018 4:03:37 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineTask {
    'use strict';

    type TineTaskType = {
        tineTaskId: string,
        tineTaskScriptName: string,
        tineTaskScriptStartDateTime: string,
        tineTaskScriptEndDateTime: string,
        dateTime: string,
        tineTaskRcd: string,
        tineTaskScriptOutcomeRcd: string,
        tineTaskPartitionKeyComputerName: string,
        tineTaskRowKeyUtcDateTime: string,
    ...
    // declaration
    declare var gettineTaskList: TineTaskType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineTaskTiles(contentElement);
        dashboard.start();
    };
    ...
    class TineTaskTiles {
        ...
        constructor(
            ...
            // init tiles
            TineTaskTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineTasks
            this.resizeCanvas(14 + 30);
            TineTaskTiles.clearCanvas('white');
            ...
            // header
            TineTaskTiles.tiles.push(header);

            // remaining tineTasks
            var lastTile: Tile;

            for (let i: number = 0; i < gettineTaskList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineTaskList[i].tineTaskScriptName,
                        40,
                        line,
                        gettineTaskList[i].tineTaskId,
                        gettineTaskList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineTaskTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineTaskTiles.tiles) {
                    for (let tile of TineTaskTiles.tiles) {
