...
// Generated Date: 9/9/2018 4:03:08 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineCacheRef {
    'use strict';

    type TineCacheRefType = {
        tineCacheRcd: string,
        tineCacheName: string,
        dateTime: string,
        tineCachePartitionKey: string,
        tineCacheRowKey: string
    };

    // declaration
    declare var gettineCacheRefList: TineCacheRefType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineCacheRefTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineCacheRefTiles {
        ...
        constructor(
            ...
            // init tiles
            TineCacheRefTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineCacheRefs
            this.resizeCanvas(14 + 30);
            TineCacheRefTiles.clearCanvas('white');
            ...
            // header
            TineCacheRefTiles.tiles.push(header);

            // remaining tineCacheRefs
            var lastTile: Tile;

            for (let i: number = 0; i < gettineCacheRefList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineCacheRefList[i].tineCacheRcd,
                        40,
                        line,
                        gettineCacheRefList[i].userId,
                        gettineCacheRefList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineCacheRefTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineCacheRefTiles.tiles) {
                    for (let tile of TineCacheRefTiles.tiles) {
