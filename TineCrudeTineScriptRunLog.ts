...
// Generated Date: 9/9/2018 4:03:31 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineScriptRunLog {
    'use strict';

    type TineScriptRunLogType = {
        tineScriptRunLogId: string,
        tineScriptRunLogScriptName: string,
        tineScriptRunLogStartDateTime: string,
        tineScriptRunLogEndDateTime: string,
        tineTaskScriptOutcomeRcd: string,
        dateTime: string,
        tineScriptRunLogPartitionKey: string,
        tineScriptRunLogRowKey: string,
        tineScriptRunLogScriptFileName: string,
        tineCreationId: string,
        tineTaskId: string
    };

    // declaration
    declare var gettineScriptRunLogList: TineScriptRunLogType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineScriptRunLogTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineScriptRunLogTiles {
        ...
        constructor(
            ...
            // init tiles
            TineScriptRunLogTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineScriptRunLogs
            this.resizeCanvas(14 + 30);
            TineScriptRunLogTiles.clearCanvas('white');
            ...
            let header: Tile =
                new Tile(
                    ...
                    line,
                    '',
                    gettineScriptRunLogList.length,
                    14 + 30,
                    15,
                    2,
                    5,
            ...
            // header
            TineScriptRunLogTiles.tiles.push(header);

            // remaining tineScriptRunLogs
            var lastTile: Tile;

            for (let i: number = 0; i < gettineScriptRunLogList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineScriptRunLogList[i].tineScriptRunLogScriptName,
                        40,
                        line,
                        gettineScriptRunLogList[i].tineScriptRunLogId,
                        gettineScriptRunLogList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineScriptRunLogTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineScriptRunLogTiles.tiles) {
                    for (let tile of TineScriptRunLogTiles.tiles) {
