...
// Generated Date: 9/9/2018 4:03:18 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineScriptLibrary {
    'use strict';

    type TineScriptLibraryType = {
        tineScriptLibraryId: string,
        tineLibraryScriptFileName: string,
        dateTime: string,
        tineLibraryScriptName: string,
        tineLibraryPartitionKey: string,
        tineLibraryRowKey: string,
        tineScriptReadinessStateRcd: string
    };

    // declaration
    declare var gettineScriptLibraryList: TineScriptLibraryType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineScriptLibraryTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineScriptLibraryTiles {
        ...
        constructor(
            ...
            // init tiles
            TineScriptLibraryTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineScriptLibrarys
            this.resizeCanvas(14 + 30);
            TineScriptLibraryTiles.clearCanvas('white');
            ...
            let header: Tile =
                new Tile(
                    ...
                    line,
                    '',
                    gettineScriptLibraryList.length,
                    14 + 30,
                    15,
                    2,
                    5,
            ...
            // header
            TineScriptLibraryTiles.tiles.push(header);

            // remaining tineScriptLibrarys
            var lastTile: Tile;

            for (let i: number = 0; i < gettineScriptLibraryList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineScriptLibraryList[i].tineLibraryScriptFileName,
                        40,
                        line,
                        gettineScriptLibraryList[i].tineScriptLibraryId,
                        gettineScriptLibraryList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineScriptLibraryTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineScriptLibraryTiles.tiles) {
                    for (let tile of TineScriptLibraryTiles.tiles) {
