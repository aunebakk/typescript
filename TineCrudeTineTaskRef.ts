...
// Generated Date: 9/9/2018 4:03:47 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineTaskRef {
    'use strict';

    type TineTaskRefType = {
        tineTaskRcd: string,
        tineTaskName: string,
        dateTime: string,
        tineTaskRefPartitionKey: string,
        tineTaskRefRowKey: string
    };

    // declaration
    declare var gettineTaskRefList: TineTaskRefType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineTaskRefTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineTaskRefTiles {
        ...
        constructor(
            ...
            // init tiles
            TineTaskRefTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineTaskRefs
            this.resizeCanvas(14 + 30);
            TineTaskRefTiles.clearCanvas('white');
            ...
            // header
            TineTaskRefTiles.tiles.push(header);

            // remaining tineTaskRefs
            var lastTile: Tile;

            for (let i: number = 0; i < gettineTaskRefList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineTaskRefList[i].tineTaskRcd,
                        40,
                        line,
                        gettineTaskRefList[i].userId,
                        gettineTaskRefList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineTaskRefTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineTaskRefTiles.tiles) {
                    for (let tile of TineTaskRefTiles.tiles) {
