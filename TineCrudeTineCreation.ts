...
// Generated Date: 9/9/2018 4:03:11 PM
// Template: sql2x.CrudeTypeScriptGenerator.OverallDesignTypeScriptTranby
module SQL2XTemplateCrudeTableTineCreation {
    'use strict';

    type TineCreationType = {
        tineCreationId: string,
        tineCreationBuilderName: string,
        tineCreationDisplayName: string,
        dateTime: string,
        tineCreationPartitionKeyFixed: string,
        tineCreationRowKey: string
    };

    // declaration
    declare var gettineCreationList: TineCreationType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineCreationTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineCreationTiles {
        ...
        constructor(
            ...
            // init tiles
            TineCreationTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineCreations
            this.resizeCanvas(14 + 30);
            TineCreationTiles.clearCanvas('white');
            ...
            // header
            TineCreationTiles.tiles.push(header);

            // remaining tineCreations
            var lastTile: Tile;

            for (let i: number = 0; i < gettineCreationList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineCreationList[i].tineCreationBuilderName,
                        40,
                        line,
                        gettineCreationList[i].tineCreationId,
                        gettineCreationList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineCreationTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineCreationTiles.tiles) {
                    for (let tile of TineCreationTiles.tiles) {
