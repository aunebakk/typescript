...
module SQL2XTemplateCrudeTableTineCache {
    ...
    // declaration
    declare var gettineCacheList: TineCacheType[]

    // client on load
    window.onload = () => {
        var contentElement: any = document.getElementById('content');
        var dashboard = new TineCacheTiles(contentElement);
        dashboard.start();
    };

    // class
    class TineCacheTiles {
        ...
        constructor(
            ...
            // init tiles
            TineCacheTiles.tiles = new Array<Tile>();
        }
        ...
        start() {
            ...
            // resize canvas to hold all tineCaches
            this.resizeCanvas(14 + 30);
            TineCacheTiles.clearCanvas('white');
            ...
            // header
            TineCacheTiles.tiles.push(header);

            // remaining tineCaches
            var lastTile: Tile;

            for (let i: number = 0; i < gettineCacheList.length; i++) {
                line++;

                lastTile =
                    new Tile(
                        gettineCacheList[i].tineCacheValue,
                        40,
                        line,
                        gettineCacheList[i].tineCacheId,
                        gettineCacheList.length,
                        14 + 30,
                        15,
                        2,
                ...
                TineCacheTiles.tiles.push(lastTile);
            }
            ...
            document.onmousemove = function (e) {
                ...
                if (TineCacheTiles.tiles) {
                    for (let tile of TineCacheTiles.tiles) {
